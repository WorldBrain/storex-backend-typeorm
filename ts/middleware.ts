...
export class ComplexCreateMiddleware implements StorageMiddleware {
    ...
    async _processCreateObject(context: StorageMiddlewareContext) {
        ...
        const batchResult = await context.next.process({
            operation: ['executeBatch', batchToExecute],
        })
    ...
    async _processExecuteBatch(context: StorageMiddlewareContext) {
        ...
        for (const step of context.operation[1] as OperationBatch) {
            ...
            const dissection = dissectCreateObjectOperation(
                ...
                {
                    generatePlaceholder: () =>
                        `${step.placeholder}-${++placeholderCount}`,
                },
            )

            const batchToExecute = convertCreateObjectDissectionToBatch(
                dissection,
            )
        ...
        const rawBatchResult = await context.next.process({
            operation: ['executeBatch', newBatch],
        })

        const processedBatchResult = { info: {} }
        for (const [placeholderName, placeholderInfo] of Object.entries(
            placeholders,
        )) {
            this._reconstructCreatedObject(
                placeholderInfo.originalObject,
                placeholderInfo.collection,
                placeholderInfo.dissection,
                rawBatchResult.info,
            )
            processedBatchResult.info[placeholderName] = {
                object: placeholderInfo.originalObject,
            }
        }
    ...
    async _reconstructCreatedObject(
        ...
        for (const step of operationDissection.objects) {
            const collectionDefiniton = this.options.storageRegistry
